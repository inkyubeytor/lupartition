...
def check_partitions(graph: nx.Graph,
                     key: Hashable,
                     parts: int,
                     lower: float,
                     upper: float,
                     partitions: List[Dict[Hashable, int]]) -> bool:
    """
    Checks the validity of a list of (l, u) partitions.
    :param graph: The networkx graph to partition. Must have `key` as a node
        attribute.
    :param key: The key for the weight of a node. The type of the weight values
        must be convertible to floats.
    ...
    :param lower: The lower weight of a partition.
    :param upper: The upper weight of a partition.
    :param partitions: The partition to check against the other parameters.
    :return: Whether the partition is valid.
    """
    return all(check_partition(graph, key, parts, lower, upper, p)
               for p in partitions)
